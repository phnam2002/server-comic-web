...
public interface FollowedComicRepository extends JpaRepository<FollowedComic, Long>, JpaSpecificationExecutor<FollowedComic>{
	...
	@Query(value = "select ufc.*  from user_followed_comics ufc where ufc.user_id = :userId order by created_at desc", nativeQuery = true)
	List<FollowedComic> findByUserId(Long userId);
	
	@Query(value = "select ufc.*  from user_followed_comics ufc where ufc.user_id = :userId && ufc.comic_id = :comicId", nativeQuery = true)
